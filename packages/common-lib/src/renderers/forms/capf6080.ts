...
const GetOtherForms = (inEvent: RawResolvedEventObject): string => {
...
export const capf6080DocumentDefinition = (
	...
	const POCText = pointsOfContact
		.map(
			(value, index) => `
${index + 1}) ${value.name}, Phone: ${formatPhone(value.phone)}, Email: ${value.email}`,
		)
		.toString();
									...
									table: {
										...
										body: [
											[
												...
												{
													text: `(Local version, generated by Event Manager on ${
										...
										body: [
											[
												...
												[
													{ text: 'Event Info:' },
													{
														qr: `https://${event.accountID}.${hostname}/eventviewer/${event.id}`,
														fit: '70',
													},
												],
											],
