...
(async () => {
  const argError = () => {
    console.error(
      "Please provide a source, account ID, expiration date, and message"
    );
    console.error(
  ...
  const check = cliargs.match(
    /^(\d{6}|SYS) (\S*) (\d{4})-(\d{2})-(\d{2}) (\d{2}):?(\d{2}) (.*)/
  );
  ...
  const alertAccount = async (account: AccountObject, db: Schema) => {
    try {
      ...
      if (from.type === NotificationCauseType.SYSTEM) {
        ...
        const member = await resolveReference(db)(account)(
          from.from!
        ).fullJoin();
  ...
  if (accountID.toLowerCase() === "all") {
    const accountsCollection = emSchema.getCollection<AccountObject>(
      "Accounts"
    );
