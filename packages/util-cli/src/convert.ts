...
(async () => {
  ...
  // @ts-ignore
  const moveFromOneToOther = <T>(
    mapFunc: (inObj: T) => T | PromiseLike<T> = identity
  ) => async (from: Collection<T>, to: Collection<T>) => {
  ...
  // @ts-ignore
  const mapFromOneToOther = <T, U>(
    mapFunc: (inObj: T) => U | PromiseLike<U>
  ) => async (from: Collection<T>, to: Collection<U>) => {
    await to.remove("true").execute();
  ...
  const v5accountsCollection = v5schema.getCollection<V5RawAccountObject>(
    "Accounts"
  );
  const v6accountsCollection = v6schema.getCollection<AccountObject>(
    "Accounts"
  );
  ...
  const v5attendanceCollection = v5schema.getCollection<
    RawAttendanceRecord<V5AttendanceRecord>
  >("Attendance");
  const v6attendanceCollection = v6schema.getCollection<
  ...
  const v5permissionsCollection = v5schema.getCollection<
    V5StoredMemberPermissions
  >("UserPermissions");
  const v6permissionsCollection = v6schema.getCollection<
    StoredMemberPermissions
  >("UserPermissions");
  ...
  const getEventPermissions = (
    ...
    ViewAccountNotifications:
      permissions.ViewAccountNotifications ??
  ...
  const getPermissions = (accountType: AccountType) => (
    permissions: V5MemberPermissions
  ) =>
