...
export const byProp = <T, K extends keyof T = keyof T>(prop: K) => (
  value: T[K]
) => (val: T) => val[prop] === value;

export const byName = byProp<{ name: string }>("name");

const roleNamesToRoles = (rolesCollection: Collection<string, Role>) => (
  roleNames: string[]
...
const getDutyPositionRoles = (guild: Guild) => (orgids: number[]) => async (
  ...
  if (hasExecutiveStaffRole(dutyPositionNames)) {
    categoryRoles.push(
      Maybe.fromValue(guild.roles.find(byName("Cadet Executive Staff")))
    );

    if (dutyPositionNames.includes("Cadet Deputy Commander")) {
      categoryRoles.push(
        Maybe.fromValue(guild.roles.find(byName("Cadet Line Staff")))
      );
    }

    if (dutyPositionNames.includes("Cadet Executive Officer")) {
      categoryRoles.push(
        Maybe.fromValue(guild.roles.find(byName("Cadet Support Staff")))
      );
    }
  }
  ...
  if (hasCACRole(dutyPositionNames)) {
    categoryRoles.push(
      Maybe.fromValue(guild.roles.find(byName("CAC Representative")))
    );
  }
  ...
  return onlyJustValues([
    ...
    ...extraRoles.map(roleName =>
      Maybe.fromValue(guild.roles.find(byName(roleName)))
    )
  ]);
};
...
const getSeniorMemberDutyPositions = (guild: Guild) => (orgids: number[]) => (
  ...
  const dutyPositions = member.dutyPositions
    .filter(
      position => position.type === "NHQ" && orgids.includes(position.orgid)
    )
    .map(get("duty"));
...
const getFlightRoles = (guild: Guild) => (member: Member): Role[] => {
  if (member.flight !== null) {
    ...
    const flightRole = Maybe.fromValue(
      guild.roles.find(isMatchingFlightRole(member.flight))
    );
...
const getTeamRoles = (guild: Guild) => (schema: Schema) => (
  account: AccountObject
) => (teamIDs: number[]) => async (member: Member): Promise<Role[]> => {
  if (teamIDs.length === 0) {
    return [];
  }
...
const get101CardCertificationRoles = (guild: Guild) => (
  schema: Schema
    ...
    ...realAchievementIDs.map(
      achv =>
        AchvIDToCertificationRole[
          achv.AchvID as keyof typeof AchvIDToCertificationRole
        ]
    )
  ];
...
const setupRoles = (guild: Guild) => (schema: Schema) => (
  account: AccountObject
) => (discordUser: GuildMember) => (teamIDs: number[]) => async (
  member: Member
) => {
  let roles: Role[];
  ...
  const isAccountMember = await asyncIterReduce<
    EitherObj<ServerError, AccountObject>,
    boolean
  >(
    (prev, memberAccount) =>
      prev ||
      Either.cata(always(false))(({ id }) => id === account.id)(memberAccount)
  ...
  if (isAccountMember) {
    if (member.seniorMember) {
      ...
      if (
        ...
        const prevCommanderRole = guild.roles.find(
          byName("Previous Cadet Commander")
        );
...
export default (client: Client) => (schema: Schema) => (guildID: string) => (
  ...
  try {
    ...
    const member = await resolveReference(schema)(account)(
      discordUser.member
    ).fullJoin();
