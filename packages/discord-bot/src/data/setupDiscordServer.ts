...
const hasRole = (roleName: string) => (guild: Guild) =>
  !!guild.roles.find(byName(roleName));
...
export const setupCAPServer = (config: ServerConfiguration) => (
  mysql: MySQLClient
) => (client: Client) => (guildId: string) => async (
  rules: Partial<DiscordSetupRules>
) => {
