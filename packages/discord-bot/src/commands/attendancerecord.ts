...
export default (client: Client) => (mysqlConn: mysql.Client) => (
  conf: ServerConfiguration
) => (parts: string[]) => async (message: Message) => {
  if (parts.length < 3) {
    await message.reply(
    ...
    await message.channel.send(
      `<@!${client.user.id}> attendancerecord 1 global`
    );
    await message.channel.send(
      `<@!${client.user.id}> attendancerecord 2 withme`
    );
    return;
  }
  ...
  const memberEither = await resolveReference(schema)(account.value)(
    maybeAdder.value.member
  );
  ...
  if (Either.isLeft(memberEither)) {
    ...
    await session.close();
    await message.channel.send(
      "Could not get member information or permissions"
    );
  ...
  if (
    effectiveManageEventPermissionForEvent(adderUser)(event) ===
  ...
  for (const member of solidMembers) {
    if (attendanceIDs.some(areMembersTheSame(member))) {
      console.log(
        `${getFullMemberName(member)} is already in attendance, skipping`
      );
      membersDuplicate++;
      return;
    }
