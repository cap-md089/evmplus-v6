...
const recordValidator = convertCAPWATCHValidator(
	validator<NHQ.ORGStatisticsRecord>(Validator) as Validator<NHQ.ORGStatisticsRecord>,
);

const orgStatisticsParse: CAPWATCHModule<NHQ.ORGStatisticsRecord> = async function* (
	backend,
	fileData,
	schema,
	isORGIDValid,
	...
	try {
		const collection = schema.getCollection<NHQ.ORGStatisticsRecord>('NHQ_ORGStatistics');

		if (!(await collection.existsInDatabase())) {
			await schema.createCollection('NHQ_ORGStatistics');
		}
		...
		for (const row of fileData) {
			if (!isORGIDValid(parseInt(row.ORGID, 10))) {
				return yield {
					type: 'Result',
			...
			const values: NHQ.ORGStatisticsRecord = {
				...row,
				ORGID: parseInt(row.ORGID, 10),
				CntDate: convertNHQDate(row.CntDate).toISOString(),
			};
...
export default orgStatisticsParse;