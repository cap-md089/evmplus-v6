...
const recordValidator = convertCAPWATCHValidator(
	validator<NHQ.PL.Lookup>(Validator) as Validator<NHQ.PL.Lookup>,
);

const plLookup: CAPWATCHModule<NHQ.PL.Lookup> = async function* (
	backend,
	fileData,
	schema,
	isORGIDValid,
	...
	try {
		const collection = schema.getCollection<NHQ.PL.Lookup>('NHQ_PL_Lookup');

		if (!(await collection.existsInDatabase())) {
			await schema.createCollection('NHQ_PL_Lookup');
		}
		...
		for (const lookup of fileData) {
			const values = {
				LookupValue: lookup.LookupValue,
				LookupType: lookup.LookupType,
				LookupID: parseInt(lookup.LookupID, 10),
			};
