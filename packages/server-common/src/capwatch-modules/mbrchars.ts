...
const recordValidator = convertCAPWATCHValidator(
	validator<NHQ.MbrChar>(Validator) as Validator<NHQ.MbrChar>,
);

const mbrCharsParse: CAPWATCHModule<NHQ.MbrChar> = async function* (
	backend,
	fileData,
	schema,
	isORGIDValid,
	...
	try {
		const collection = schema.getCollection<{ [key: string]: unknown }>('NHQ_MbrChars');

		if (!(await collection.existsInDatabase())) {
			await schema.createCollection('NHQ_MbrChars');
		}
		...
		for (const row of fileData) {
			if (!isORGIDValid(capidMap[parseInt(row.CAPID, 10)])) {
				return yield {
					type: 'Result',
			...
			const values = {
				...row,
				CAPID: parseInt(row.CAPID, 10),
				...(row.DateMod ? { DateMod: convertNHQDate(row.DateMod).toISOString() } : {}),
			};
