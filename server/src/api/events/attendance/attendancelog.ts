...
export default asyncErrorHandler(async (req: AccountRequest<{ id: string }>, res: Response) => {
	...
	for await (const member of req.account.getMembers()) {
		if(!member.seniorMember) {
			memberInformation.push([
				...
				member.memberRank,
				member.id.toString(),
				member instanceof CAPNHQMember ? 'exp date' : 'N/A',
				' ',
				member.flight
			]);
		}
	...
	const formattedMemberInformation = [
		...
		...memberInformation.map((elem, i) => [
			{ text: elem[0], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' },
			{ text: elem[1], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' },
			{ text: elem[2], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' },
			{ text: elem[3], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' },
			{ text: elem[4], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' },
			{ text: elem[5], bold: false, fillColor: i % 2 == 0 ? 'lightgrey' : 'white' }
		])
	];
